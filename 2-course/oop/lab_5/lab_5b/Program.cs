...
namespace lab_4
{
    class Program
    {
        ...
        {
            Sort<int> sort = new Sort<int>(3,2,1,1,165);
            ...
            date.Add(d1); date.Add(d2); date.Add(d3); date.Add(d4);
            date.Sort((a,b) => a.CompareTo(b));
            
            for (int i = 0; i < date.Count; i++) {
                Console.Write(date[i].day + "." + date[i].month + "." + date[i].year + " ");
            }

            // Console.Write("Відсортовані дати: ");
            // for (int i = 0; i < sort3.data.Count; i++) {
            ...
            // for (int i = 0; i < sort4.data.Count; i++) {
